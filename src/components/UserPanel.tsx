...
import { Box, Flex, Button, Stack, Text, Heading, Divider, Image, HStack, CloseButton, Spacer } from "@chakra-ui/react"
...
export function UserPanel({ address }: { address: string }) {
  const { graphAddress, setGraphAddress, setSelectedAddress } = useContext(GraphContext);
      ...
      <Box bg={'white'} color={'black'} margin={1} padding={1}>
        <Stack spacing={4} margin={2}>
          <Flex direction={'column'} >
            ...
            {identity.avatar &&
              <Box display={'flex'}>
                <Box bg={'black'} padding={.5} borderRadius={'lg'} >
                  <Box bg={'white'} padding={.5} borderRadius={'lg'}>
                    <Image src={identity.avatar} alt={''} boxSize={'200px'} borderRadius={'lg'} />
                  </Box>
                </Box>
              </Box>
            }
